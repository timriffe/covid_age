# -*- coding: utf-8 -*-
"""
Created on Mon Jan  9 10:15:37 2023
...
pathSpecified = "N:/COVerAGE-DB/Automation/Denmark"  #Location from where the current day's automation data has to be extracted
timestr = time.strftime("%Y%m%d")


files=pd.read_csv('N:/COVerAGE-DB/Automation/Denmark/DenmarkRows.csv') #CSV File where the previous day's automation data is stored
print(len(files))
...
if (len(files))!=len(list_of_files): #Compare previous day's data in csv file and current day's data
    ...
    pd.DataFrame({'files':list_of_files}).to_csv('N:/COVerAGE-DB/Automation/Denmark/DenmarkRows.csv') #Updating the Previous day's CSV Files with current day's records
    print(len(list_of_files))
    print(len(files))
    wbkName = 'N:/COVerAGE-DB/Automation/Python-Dashboard/PythonDashboard.xlsx' #Indicating the path and filename of Python Dashboard
    wbk = openpyxl.load_workbook(wbkName) #Opening the Python Dashboard
