# -*- coding: utf-8 -*-
"""
Created on Mon Jan  9 10:00:08 2023
...
pathSpecified = "N:/COVerAGE-DB/Automation/Bulgaria"  #Location from where the current day's automation data has to be extracted
timestr = time.strftime("%Y%m%d")


files=pd.read_csv('N:/COVerAGE-DB/Automation/Bulgaria/BulgariaRows.csv') #CSV File where the previous day's automation data is stored
print(len(files))
...
wks.cell(row=4, column=6).value = wks.cell(row=4, column=5).value #Writing the previous day's record
#myRow = 9
for myCol in range(14,7,-1):
    wks.cell(row=4, column=myCol).value = wks.cell(row=4, column=myCol-1).value
...
wks.cell(row=4, column=7).value = "FALSE"
wbk.save(wbkName)
wbk.close
...
if (len(files))!=len(list_of_files): #Compare previous day's data in csv file and current day's data
    ...
    pd.DataFrame({'files':list_of_files}).to_csv('N:/COVerAGE-DB/Automation/Bulgaria/BulgariaRows.csv') #Updating the Previous day's CSV Files with current day's records
    print(len(list_of_files))
    print(len(files))
    wbkName = 'N:/COVerAGE-DB/Automation/Python-Dashboard/PythonDashboard.xlsx' #Indicating the path and filename of Python Dashboard
    wbk = openpyxl.load_workbook(wbkName) #Opening the Python Dashboard
    wks = wbk["dash-view"] #Choosing the first worksheet in the excel workbook
    wks.cell(row=4, column=5).value = len(list_of_files) #Writing the current day's record in Dashboard
    wks.cell(row=4, column=7).value = "TRUE" #Indicate DATA DOWNLOADED ON THE PARTICULAR DATE
