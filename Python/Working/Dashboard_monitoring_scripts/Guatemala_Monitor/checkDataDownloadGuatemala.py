...
wks.cell(row=7, column=6).value = wks.cell(row=7, column=5).value #Writing the previous day's record
#myRow = 9
for myCol in range(14,7,-1):
    wks.cell(row=7, column=myCol).value = wks.cell(row=7, column=myCol-1).value
...
wks.cell(row=7, column=7).value = "FALSE"
wbk.save(wbkName)
wbk.close
...
if (len(files))!=len(list_of_files): #Compare previous day's data in csv file and current day's data
    ...
    wbk = openpyxl.load_workbook(wbkName) #Opening the Python Dashboard
    wks = wbk["dash-view"] #Choosing the first worksheet in the excel workbook
    wks.cell(row=7, column=5).value = len(list_of_files) #Writing the current day's record in Dashboard
    wks.cell(row=7, column=7).value = "TRUE" #Indicate DATA DOWNLOADED ON THE PARTICULAR DATE
