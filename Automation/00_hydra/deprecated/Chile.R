...
## MK: 27.03.2023 deaths by region and age are not published in the below links - website is down for few months. 
## total deaths by age is published in github repo. so I added these data for now

## here the data are cumulative already. 

deaths_raw <- read_csv("https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto10/FallecidosEtario.csv")

Deaths <- deaths_raw |> 
  rename(Age = "Grupo de edad") |> 
  pivot_longer(cols = -c("Age"),
               names_to = "Date",
               values_to = "Value") |> 
  mutate(Age = str_extract(Age, "\\d+"),
         Age = case_when(Age == "39" ~ "0",
                         TRUE ~ Age),
