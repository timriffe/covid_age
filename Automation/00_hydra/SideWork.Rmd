...
seq_dates <- format(seq(from = as.Date("2020/07/20"), to = as.Date("2021/05/20"), by = "1 days"), "%Y%m%d")
...
files <- list.files(path = trial_path,
                    pattern = ".ods",
                    full.names = TRUE) 
...
total_pre2022 <- "Hoja3" ## total sheet from 04.01.2021 till 
...
out_totals <- df_paths_sheets(files_df, total_sheet)

## VAX1: SHEET 3 ##

out_VAX1 <- df_paths_sheets(files_df, dose_1)

## VAX2: SHEET 4 ##

out_VAX2 <- df_paths_sheets(files_df, dose_2)
...
processed_total <- out_totals %>% 
  select(Date = id, 
         ...
         Vaccination2 = K,
         Vaccination3 = L) %>% 
   mutate(Date = str_extract(Date, "\\d+.ods"),
         Date = str_remove(Date, ".ods"),
         Date = ymd(Date),
          across(.cols = -c("Region", "Date"), ~str_extract(.x, "\\d+"))) %>% 
...
## FUNCTIONS TO PROCESS VAX DOSES DATA ==========================

vax_process <- function(tbl, vax_number){
  
  tbl %>% 
  pivot_longer(-c("Region", "Date"), names_to= "Age", values_to= "Value") %>%
...
processed_vax1 <- out_VAX1 %>% 
  ...
  filter(!is.na(Date),
         `80` != "1") %>% 
  vax_process("Vaccination1")


### Vaxx 2

processed_vax2 <- out_VAX2 %>% 
  select(Region = A,
         Date = id,
         `80`= B ,
         `70`= D, 
